...
/*
 * This simulation is identical to the original on, except for the different
 * endpoint names and for the absence of `.randomized` at the warmup part.
 */
class RinhaBackendSimulation
  extends Simulation {

  val httpProtocol = http
    .baseUrl("http://lb:9999")
  ...
  val criacaoEConsultaPessoas = scenario("Criação E Talvez Consulta de Pessoas")
    ...
    .exec(
      http("criação")
      .post("/users").body(StringBody("#{payload}"))
